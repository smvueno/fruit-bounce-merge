  ...
  return (
    <div className="fixed inset-0 bg-white/30 backdrop-blur-md flex flex-col items-center justify-center p-4 z-50 text-white animate-fade-in font-['Fredoka']">
      ...
      <div className="bg-white/95 backdrop-blur-2xl border border-white/50 text-gray-800 rounded-[2.5rem] px-8 py-6 w-full max-w-lg text-center shadow-[0_20px_60px_rgba(0,0,0,0.3)] transform scale-100 animate-pop flex flex-col items-center max-h-[85vh] overflow-y-auto min-h-0">
        ...
        {/* Score with floating badge */}
        <div className="relative w-full flex items-center justify-center mb-4 shrink-0">
          <div className="text-5xl font-bold text-gray-900 font-mono tracking-tighter drop-shadow-sm">
            {stats.score.toLocaleString()}
          </div>

          {/* Floating badge - Only show if qualifies for leaderboard AND hasn't saved yet */}
          {isNewHigh && !hasSaved && (
            <div className="absolute -top-4 right-1/4 animate-breathe">
              <div className={`font-bold px-3 py-1 inline-flex items-center gap-1 text-sm ${isNumberOne ? 'text-yellow-500' : 'text-blue-600'}`}>
          ...
          <div className="w-full mb-4 shrink-0 animate-fade-in">
            ...
            <div className="flex gap-2 w-full">
              <input
                ...
                maxLength={10}
                className="flex-1 min-w-0 bg-slate-100 border-2 border-slate-200 rounded-2xl px-3 py-3 font-bold text-slate-800 text-base focus:outline-none focus:border-blue-500 transition-colors placeholder:text-slate-300"
              />
              <button
                onClick={handleSave}
                disabled={name.length === 0}
                className="bg-blue-500 text-white font-bold px-4 py-3 rounded-2xl shadow-md active:scale-95 disabled:opacity-50 disabled:active:scale-100 shrink-0"
              >
                SAVE
              </button>
            </div>
        ...
        {/* Action Buttons - Combined on one row */}
        <div className="w-full flex gap-3 shrink-0">
          <button
            onClick={onMenu}
            disabled={isCoolingDown}
            className={`flex-1 font-bold text-base md:text-xl py-3 md:py-6 rounded-2xl md:rounded-3xl transition-all flex items-center justify-center gap-2 ${isCoolingDown
          ...
          <button
            onClick={onRestart}
            disabled={isCoolingDown}
            className={`flex-1 font-bold text-base md:text-xl py-3 md:py-6 rounded-2xl md:rounded-3xl shadow-[0_8px_16px_rgba(245,158,11,0.3)] transition-all flex items-center justify-center gap-2 ${isCoolingDown
