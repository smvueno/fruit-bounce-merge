...
export const GameHUD: React.FC<GameHUDProps> = React.memo(({ score, playTime, maxTier, nextFruit, savedFruit, onSwap }) => {
    ...
    return (
        <div className="w-full h-full flex flex-col justify-center px-4 z-30 pointer-events-none font-['Fredoka'] pb-1">
            <div className="flex justify-between items-end w-full">
                <div className="flex flex-col gap-1 items-start relative">
                    <div className="text-gray-800 text-xs font-bold tracking-widest uppercase mb-0 opacity-90">SCORE</div>
                    <div
                        id="hud-score-display"
                        className="text-3xl font-bold text-[#1a1a1a] leading-none drop-shadow-sm transition-transform duration-300 placeholder-opacity-100 origin-left scale-100"
                        style={{ WebkitTextStroke: '0px transparent' }}
                    >
                        {animatedScore.toLocaleString()}
                    </div>
                    <div className="flex items-center gap-2 text-lg font-bold text-gray-800 mt-1">
                        <Clock size={16} className="text-gray-900" strokeWidth={3} />
                        <span>{formatTime(playTime)}</span>
                    </div>
                    <div className="flex items-center gap-3 text-lg font-bold text-gray-800 mt-0">
